...
function sendChat() {
  var chatMsg = new ChatMessage(myName, "it is chat");
  var chatJson = JSON.stringify(chatMsg);
  console.log(`sending chat ${chatJson}`);
  for (const [name, peer] of Object.entries(peers)) {
    console.log(`${JSON.stringify(peer)}`);
    // if(!peer || !peer.dataChannel) return;

    peer.dataChannel.send(chatJson);
  }
...
function onReceiveMessageFromPeer(peerName) {
  return async (event) => {
...
function onReceiveChatMessage(event) {
...
function appendChatMessage(msg) {
  chatBox.innerHTML += `<p>[${msg.timestamp}] ${msg.sender}: ${msg.msg}</p>`;
}

function onDataChannelStateChange(peerName) {
  return (event) => {
